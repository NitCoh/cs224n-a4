...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(embed_size + hidden_size, hidden_size)  # input size (embed + hidden) = (word + attention)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 1. when we put -inf where the <pad> in the sentence, we get softmax of -inf included on the attention scores
        # corresponds to the <pad> words, result in weight that is 0.
        # Then explain (in one or two sentences) why it is necessary to use the masks in this way.
        # 2. In order to not give any attention to the padded words, thus compute the attention
        # over the relevant words in the sentence only.
